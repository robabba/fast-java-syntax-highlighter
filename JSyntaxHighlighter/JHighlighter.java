...
public class JHighlighter extends JTextPane {
	...
	private static final String SYNTAX_NUMERIC_RULE = "^\\d+$";
	private SyntaxStyle SYNTAX_STYLE = new SyntaxStyle();
	

	/** 
	 * Create a new JHighlighter Object with a predefined language.
	 * @param language
	 */
	public JHighlighter(Language language){
	 ...
	 * @param theme
	 */
	public JHighlighter(Language language, Theme theme){		
		this.setMargin(new Insets(2,10,0,0));
		this.setTheme(theme);
		...
		String dir = classLocation.getAbsolutePath();
		
		System.out.println(dir);
		
		try{
		    Meslo = Font.createFont(Font.TRUETYPE_FONT, new File("src/JHighlighter/Fonts/MesloLGL-Regular.ttf")).deriveFont(12f);
			GraphicsEnvironment ge = GraphicsEnvironment.getLocalGraphicsEnvironment();
			ge.registerFont(Font.createFont(Font.TRUETYPE_FONT, new File("src/JHighlighter/Fonts/MesloLGL-Regular.ttf")));
		}catch(IOException e){
			e.printStackTrace();
	...
	/** 
	 * Create a new JHighlighter Object with a custom XML file.
	 * @param language
	 */
	public JHighlighter(String customFile){
